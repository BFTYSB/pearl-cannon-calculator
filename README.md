...
> 🛑 **暂停开发公告**
> 
> 因本人个人原因，本模组暂时停止开发。当前仓库仅保留初版测试代码，恢复时间未定。
...
> ℹ️ 200 tick 仅为代码内默认参考值，并非 Minecraft 的实际硬限制。珍珠实际飞行时长取决于距离、速度与落地条件，FTL 远程炮可能远超此值。
>
> ℹ️ `air_drag_modifier` 默认值为 1.0，但其实际生效取决于是否同时启用了均匀空气阻力（uniform air drag）组件。若该组件未启用，modifier 不参与运动计算，阻力恒为基础值。
```
